...
class Map3(Map):
    def __init__(self, num_players, rng=None):
        ...
        self.create_cities_and_routes(
            num_players
        )  # Populate cities, offices, and routes etc., specifically for Map1
        self.assign_starting_bonus_markers()

        self.map_width = 1800
        self.map_height = 1800

        # Keep city names alphabetized because drawing code uses sorted route keys.
        self.bonus_marker_positions = {
            ("Carlisle", "IsleOfMan"): (351, 501),
            ("Conway", "IsleOfMan"): (200, 630),
            ("Chester", "Conway"): (336, 780),
            ("Conway", "Montgomery"): (189, 911),
            ("Hereford", "Montgomery"): (526, 1000),
            ("Montgomery", "Pembroke"): (275, 1106),
            ("Cardiff", "Pembroke"): (132, 1257),
            ("Cardiff", "Montgomery"): (440, 1111),
            ("Carlisle", "Richmond"): (587, 372),
            ("Carlisle", "Durham"): (838, 323),
            ("Lancaster", "Richmond"): (805, 563),
            ("Durham", "Lancaster"): (1081, 638),
            ("Durham", "Newcastle"): (1217, 322),
            ("Chester", "Hereford"): (628, 713),
            ("Hereford", "Lancaster"): (822, 846),
            ("Coventry", "Hereford"): (632, 1011),
            ("Coventry", "Nottingham"): (767, 926),
            ("Nottingham", "York"): (1133, 714),
            ("Durham", "York"): (1316, 544),
            ("Norwich", "Nottingham"): (1089, 911),
            ("Cambridge", "Norwich"): (1183, 1018),
            ("Cambridge", "Ipswich"): (1124, 1165),
            ("Cambridge", "Coventry"): (875, 1138),
            ("Cambridge", "London"): (1230, 1162),
            ("Cardiff", "Coventry"): (514, 1134),
            ("Cardiff", "Oxford"): (572, 1303),
            ("London", "Oxford"): (1032, 1348),
            ("Oxford", "Salisbury"): (792, 1379),
            ("Bristol", "Salisbury"): (534, 1457),
            ("Plymouth", "Salisbury"): (439, 1600),
            ("Salisbury", "Southhampton"): (773, 1642),
            ("Calais", "Southhampton"): (1174, 1675),
            ("Calais", "Canterbury"): (1229, 1485),
            ("Canterbury", "London"): (1152, 1353),
        }
        if num_players > 3:
            self.bonus_marker_positions.update(
                {
                    ("Edinbaurgh", "Glasgom"): (272, 39),
                    ("Dunbar", "Edinbaurgh"): (605, 145),
                    ("Dunbar", "Newcastle"): (982, 81),
                    ("Falkirk", "Glasgom"): (292, 232),
                    ("Carlisle", "Falkirk"): (547, 265),
                    ("Carlisle", "Chester"): (534, 526),
                }
            )
        else:
            self.bonus_marker_positions[("Carlisle", "Newcastle")] = (721, 202)
    ...
    def create_cities_and_routes(self, num_players):
        ...
        self.upgrade_cities.append(
            Upgrade(
                "York",
                "Keys",
                York.x_pos,
                York.y_pos - UPGRADE_Y_AXIS_OFFSET,
                width=York.width,
                height=York.height - 30,
            )
        )
        self.upgrade_cities.append(
            Upgrade(
                ...
                "Privilege",
                London.x_pos,
                London.y_pos - UPGRADE_Y_AXIS_OFFSET,
                width=London.width,
                height=London.height - 30,
            )
        )
        self.upgrade_cities.append(
            Upgrade(
                ...
                "Book",
                Norwich.x_pos,
                Norwich.y_pos - UPGRADE_Y_AXIS_OFFSET,
                width=Norwich.width,
                height=Norwich.height - 30,
            )
        )
        self.upgrade_cities.append(
            Upgrade(
                ...
                "Actions",
                Chester.x_pos,
                Chester.y_pos - UPGRADE_Y_AXIS_OFFSET,
                width=Chester.width,
                height=Chester.height - 30,
            )
        )
        self.upgrade_cities.append(
            Upgrade(
                ...
                "Bank",
                Newcastle.x_pos,
                Newcastle.y_pos - UPGRADE_Y_AXIS_OFFSET,
                width=Newcastle.width,
                height=Newcastle.height - 30,
            )
        )
        ...
        x_specialprestigepoints_box_size = (
            circle_size * 4 + SPACING * 4
        )  # Using SPACING*3 because there's no spacing after the last circle

        # Determine the starting x-position for the rectangle, ensuring it's centered
        x_start_pos = (Plymouth.x_pos + (Plymouth.width / 2)) - (
            x_specialprestigepoints_box_size / 2
        )
        ...
        self.specialprestigepoints = Upgrade(
            "Plymouth",
            "SpecialPrestigePoints",
            x_start_pos,
            y_start_pos,
            width=x_specialprestigepoints_box_size,
            height=circle_size + SPACING * 2,
        )
        ...
        if num_players > 3:
            ...
            self.routes.append(
                Route(
                    [Carlisle, IsleOfMan], 3, required_circles=2, color=DARK_BLUE, region="Scotland"
                )
            )
        else:
            self.routes.append(
                Route([Carlisle, IsleOfMan], 3, required_circles=2, permanent_bm_type="MoveAny2")
            )
            self.routes.append(Route([Carlisle, Newcastle], 4))

        #  BROWN
        self.routes.append(
            Route([Conway, IsleOfMan], 3, required_circles=2, color=BLACKISH_BROWN, region="Wales")
        )
        ...
        self.routes.append(
            Route(
                [Southhampton, Calais],
                3,
                permanent_bm_type="Place2ScotlandOrWales",
                required_circles=2,
            )
        )
        self.routes.append(
            Route([Canterbury, Calais], 3, permanent_bm_type="MoveAny2", required_circles=1)
        )
