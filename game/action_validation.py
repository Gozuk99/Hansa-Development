...
def state_summary(game) -> str:
    return (
        f"seed={game.seed}, turn={game.turn_number}, round={game.round_number}, "
...
def validate_action_state(game) -> ActionValidationResult:
    ...
    for index in enabled:
        ...
        try:
            ...
            _fail(game, f"enabled action failed: {type(error).__name__}: {error}", index, action)
        try:
