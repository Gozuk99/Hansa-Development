...
class BoardData:
    ...
    def load_game_info_JSON(self, game, game_info_JSON):
        ...
        game.waiting_for_bm_tribute_trading_post = game_info_JSON[
            "waiting_for_bm_tribute_trading_post"
        ]
        ...
        game.pending_britannia_place2 = game_info_JSON.get("pending_britannia_place2", False)

        game.cardiff_priv = (
            game.players[game_info_JSON["cardiff_priv"] - 1]
            if game_info_JSON["cardiff_priv"]
            else None
        )
        game.carlisle_priv = (
            game.players[game_info_JSON["carlisle_priv"] - 1]
            if game_info_JSON["carlisle_priv"]
            else None
        )
        game.london_priv = (
            game.players[game_info_JSON["london_priv"] - 1]
            if game_info_JSON["london_priv"]
            else None
        )
        ...
        for i, color in enumerate(colors):
            ...
            if owner_order is not None:
                game.selected_map.specialprestigepoints.circle_data[i]["owner"] = game.players[
                    owner_order - 1
                ]
            else:
        ...
        if game_info_JSON["tile_owner_DisplaceAnywhere"] is not None:
            game.DisplaceAnywhereOwner = game.players[
                game_info_JSON["tile_owner_DisplaceAnywhere"] - 1
            ]
        ...
        if game_info_JSON["tile_owner_+1IncomeIfOthersIncome"] is not None:
            game.OneIncomeIfOthersIncomeOwner = game.players[
                game_info_JSON["tile_owner_+1IncomeIfOthersIncome"] - 1
            ]

        if game_info_JSON["tile_owner_+1DisplacedPiece"] is not None:
            game.OneDisplacedPieceOwner = game.players[
                game_info_JSON["tile_owner_+1DisplacedPiece"] - 1
            ]

        if game_info_JSON["tile_owner_+4PtsPerOwnedCity"] is not None:
            game.FourPtsPerOwnedCityOwner = game.players[
                game_info_JSON["tile_owner_+4PtsPerOwnedCity"] - 1
            ]

        if game_info_JSON["tile_owner_+7PtsPerCompletedAbility"] is not None:
            game.SevenPtsPerCompletedAbilityOwner = game.players[
                game_info_JSON["tile_owner_+7PtsPerCompletedAbility"] - 1
            ]
    ...
    def load_city_info_JSON(self, game, city_info_JSON):
        ...
        for i, city in enumerate(game.selected_map.cities):
            if (
                city.name == city_info_JSON[i]["city_name"]
                and city.color == city_colors[city_info_JSON[i]["city_color"]]
            ):
    ...
    def load_player_info_JSON(self, game, player_info_JSON):
        ...
        color_names = {"green": GREEN, "blue": BLUE, "purple": PURPLE, "red": RED, "yellow": YELLOW}
        for player_data in player_info_JSON:
            ...
            for player in game.players:
                if (
                    player.color == player_data_color
                    and player.order == player_data["player_order"]
                ):
    ...
    def fill_game_info_JSON(self, game):
        ...
        if game.selected_map.specialprestigepoints.circle_data:
            ...
            for i, color in enumerate(colors):
                ...
                if "owner" in circle_data and circle_data["owner"]:
                    special_prestige_points_info_JSON[f"special_prestige_points_{color}"] = (
                        circle_data["owner"].order
                    )

        bonus_marker_pool_info_JSON = {}
        for i in range(len(game.selected_map.bonus_marker_pool)):
            bonus_marker_pool_info_JSON[f"bonus_marker{i + 1}"] = (
                game.selected_map.bonus_marker_pool[i]
                if game.selected_map.bonus_marker_pool[i]
                else None
            )
        ...
        tile_owner_info_JSON = {
            ...
            if game.DisplaceAnywhereOwner
            else None,
            "tile_owner_+1Action": game.OneActionOwner.order if game.OneActionOwner else None,
            "tile_owner_+1IncomeIfOthersIncome": game.OneIncomeIfOthersIncomeOwner.order
            if game.OneIncomeIfOthersIncomeOwner
            else None,
            "tile_owner_+1DisplacedPiece": game.OneDisplacedPieceOwner.order
            if game.OneDisplacedPieceOwner
            else None,
            "tile_owner_+4PtsPerOwnedCity": game.FourPtsPerOwnedCityOwner.order
            if game.FourPtsPerOwnedCityOwner
            else None,
            "tile_owner_+7PtsPerCompletedAbility": game.SevenPtsPerCompletedAbilityOwner.order
            if game.SevenPtsPerCompletedAbilityOwner
            else None,
        }
    ...
    def fill_city_info_JSON(self, game):
        ...
        for city in game.selected_map.cities:
            city_data_JSON = {
                ...
                "city2_upgrade": city.upgrade_city_type[1]
                if len(city.upgrade_city_type) > 1
                else None,
                "city_tribute0": city.tributed_players[0] if city.tributed_players else None,
                "city_tribute1": city.tributed_players[1]
                if len(city.tributed_players) > 1
                else None,
                "city_tribute2": city.tributed_players[2]
                if len(city.tributed_players) > 2
                else None,
                "city_tribute3": city.tributed_players[3]
                if len(city.tributed_players) > 3
                else None,
            }
            ...
            for office in city.offices:
                office_data = {
                    ...
                    "office_controller": player_colors[office.controller.color]
    ...
    def fill_route_info_JSON(self, game):
        ...
        for route in game.selected_map.routes:
            route_data_JSON = {
                ...
                "route_block_marker_owners": [owner.order for owner in route.block_marker_owners],
            }
    ...
    def fill_player_info_JSON(self, game):
        ...
        for player in game.players:
            player_data_JSON = {
                ...
                "player_mission_card_cities": [city for city in player.mission_card]
                if player.mission_card
                else None,
            ...
            for used_bm in player.used_bonus_markers:
                player_data_JSON[f"player_used_bm_{used_bm.type}"] = (
                    used_bm.type if used_bm else None
                )
