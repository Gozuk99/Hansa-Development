...
@dataclass
class DrawLayout:
    """Clickable regions produced by a render pass, kept outside engine state."""
...
def draw_upgrade_on_map(window, upgrade_type):
    ...
    if upgrade_type.upgrade_type == "SpecialPrestigePoints":
        draw_special_prestige_points(window, upgrade_type)
        return
...
def draw_completed_cities_indicator(win, game):
    ...
    draw_text(
        win,
        "Completed Cities",
        selected_map.max_full_cities_x_pos,
        selected_map.max_full_cities_y_pos + SQUARE_SIZE + 5,
        FONT_SMALL,
        BLACK,
        centered=False,
    )
...
def draw_bonus_markers(win, selected_map):
    ...
    for route in routes:
        ...
        if route.permanent_bonus_marker:
            ...
            # If the position exists, call the draw method on the bonus marker
            if bonus_marker_pos:
                draw_board_bonus_markers(
                    win, route.permanent_bonus_marker, bonus_marker_pos, color=BLUE
                )
...
def draw_city_offices(win, city):
    ...
    for office in city.offices:
        if office.shape == "square":
            ...
            draw_circle_office(
                win, office, start_x, start_y - CIRCLE_RADIUS // 2
            )  # Adjust Y for circle center
            start_x += CIRCLE_RADIUS * 2 + SPACING
    ...
    if post.required_shape == "circle":
        ...
        if post.owner_piece_shape is None or post.owner_piece_shape == "square":
            draw_shape(
                ...
                "rectangle",
                post.square_color,
                post_x - SQUARE_SIZE // 2,
                post_y - SQUARE_SIZE // 2,
                SQUARE_SIZE,
                SQUARE_SIZE,
            )
    if post.blocked_bm:
...
def draw_scoreboard(win, players, start_x, start_y):
    ...
    for index, player in enumerate(players):
        ...
        text_position = (
            start_x + 10,
            start_y + y_offset + (index * 25),
        )  # Y position adjusted here
        win.blit(text_surface, text_position)
...
def draw_bonus_marker_pool(win, game):
    ...
    draw_shape(
        ...
        start_x,
        start_y,
        bonus_marker_pool_text_box_width,
        bonus_marker_pool_text_box_height,
    )
    ...
    draw_text(
        win,
        "Face-down supply",
        start_x + bonus_marker_pool_text_box_width // 2,
        start_y + 55,
        FONT_SMALL,
        BLACK,
        centered=True,
...
def redraw_window(win, game, legal_actions=()):
    ...
    layout.tile_rects = draw_tiles(
        win,
        game.tile_pool,
        selected_map.map_width + 500,
        selected_map.map_height - 170,
    )
...
def draw_player_board(window, player, current_player, game=None, legal_actions=()):
    ...
    border_color = WHITE if player == current_player else BLACK
    pygame.draw.rect(
        window,
        border_color,
...
def draw_city_keys_section(window, board):
    ...
    for i, value in enumerate(CITY_KEYS_MAX_VALUES):
        ...
        draw_text(
            window,
            str(value),
            board.x + 10 + i * (SQUARE_SIZE + 5) + SQUARE_SIZE // 2,
            board.y + 10 + SQUARE_SIZE // 2,
            FONT_PLAYERBOARD,
            BLACK,
            centered=True,
        )
...
def draw_privilegium_section(window, board):
    ...
    draw_text(
        window, "Privilege", board.x + 10, privilege_y + SQUARE_SIZE + 5, FONT_PLAYERBOARD, BLACK
    )
...
def draw_bonus_markers_section(window, board):
    ...
    for i, bm in enumerate(board.player.bonus_markers):
        ...
        draw_board_bonus_markers(window, bm, (bm_x, bm_start_y))


def draw_liber_sophiae_section(window, board):
    # Draw "Liber Sophiae" section (circles)
    colors = [WHITE, ORANGE, PINK, BLACK]
    start_x = board.x + len(colors) * 43  # buffer after Privilege

    circle_label = FONT_PLAYERBOARD.render("Liber Sophiae", True, BLACK)
    circle_section_width = len(BOOK_OF_KNOWLEDGE_MAX_VALUES) * (
        CIRCLE_RADIUS * 2 + 5
    )  # total width of all circles combined
    ...
    for i, value in enumerate(BOOK_OF_KNOWLEDGE_MAX_VALUES):
        ...
        draw_shape(
            ...
            "circle",
            color,
            start_x + i * (CIRCLE_RADIUS * 2 + 5),
            board.y + 10 + CIRCLE_RADIUS,
            width=CIRCLE_RADIUS,
        )
        draw_text(
            window,
    ...
    draw_text(
        ...
        board.y + 10 + CIRCLE_RADIUS * 2 + 5,
        FONT_PLAYERBOARD,
        BLACK,
    )


def _board_right_tracks_x(board):
    start_x = board.x + 4 * 43
    circle_label_width = FONT_PLAYERBOARD.size("Liber Sophiae")[0]
    circle_section_width = len(BOOK_OF_KNOWLEDGE_MAX_VALUES) * (CIRCLE_RADIUS * 2 + 5)
    start_x += (circle_section_width - circle_label_width) // 2
    return start_x + len(BOOK_OF_KNOWLEDGE_MAX_VALUES) * (CIRCLE_RADIUS * 2 + 5)
...
def draw_tiles_section(window, board):
    ...
    tiles_start_x = (
        _board_right_tracks_x(board)
        - (len(BOOK_OF_KNOWLEDGE_MAX_VALUES) * (CIRCLE_RADIUS * 2 + 5))
        - CIRCLE_RADIUS
    )
...
def draw_actiones_section(window, board):
    ...
    actions_y = board.y + 10
    start_x = _board_right_tracks_x(board)
    ...
    for i, value in enumerate(ACTIONS_MAX_VALUES):
        ...
        draw_text(
            window,
            str(value),
            start_x + i * (SQUARE_SIZE + 5) + SQUARE_SIZE // 2,
            actions_y + SQUARE_SIZE // 2,
            FONT_PLAYERBOARD,
            BLACK,
            centered=True,
        )

    draw_text(window, "Actiones", start_x, actions_y + SQUARE_SIZE + 5, FONT_PLAYERBOARD, BLACK)
...
def draw_bank_section(window, board):
    ...
    actions_y = board.y + 10
    start_x = _board_right_tracks_x(board)
    bank_y = actions_y + SQUARE_SIZE + 5 + FONT_PLAYERBOARD.get_height() + 5
    ...
    for i, value in enumerate(BANK_MAX_VALUES):
        ...
        draw_text(
            ...
            str(value),
            start_x + i * (SQUARE_SIZE + 5) + SQUARE_SIZE // 2,
            bank_y + SQUARE_SIZE // 2,
            FONT_PLAYERBOARD,
            BLACK,
            centered=True,
        )

    draw_text(window, "Bank", start_x, bank_y + SQUARE_SIZE + 5, FONT_PLAYERBOARD, BLACK)
...
def draw_used_bm_section(window, board, current_player):
    ...
    used_bm_x = _board_right_tracks_x(board) + 6 * (SQUARE_SIZE + 5)
    ...
    if board.player != current_player:
        draw_text(
            window,
            f"{len(board.player.used_bonus_markers)} face-down",
            used_bm_x,
            used_bm_y,
            FONT_SMALL,
            BLACK,
    ...
    for i, bm in enumerate(board.player.used_bonus_markers):
        draw_text(window, bm.type, used_bm_x, used_bm_y + i * 22, FONT_SMALL, BLACK)
...
def draw_context_action_buttons(window, board, game, legal_actions):
    # Starting position for the Income label
    income_x = board.x + board.width - 560
