...
| Displaced player may decline additional pieces | Game Play C | `finish_displacement` | Finish Displacement control | Decline after mandatory displaced-piece placement | **Implemented** | The displaced piece must be replaced; any still-unclaimed extras may then be declined without skipping held pieces. |
...
| Move any 2 tradesmen | `waiting_for_bm_move_any_2`; shared move engine | Own/opponent post interactions, Finish Move control, then destinations | Own piece, opponent preservation, early finish, swap-capable vacated destinations | **Implemented** | Up to two pieces of either ownership may be moved without displacement or action cost. |
